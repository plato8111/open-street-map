...
function debugSupabaseAccess() {
  debug.group('🔍 Supabase Plugin Debug Information');

  // Check wwLib availability and properties
  debug.log('wwLib available:', typeof wwLib !== 'undefined');
  if (typeof wwLib !== 'undefined') {
    debug.log('Available wwLib properties:', Object.keys(wwLib));

    // Check wwPlugins (MOST IMPORTANT for Supabase)
    debug.log('wwLib.wwPlugins available:', typeof wwLib.wwPlugins !== 'undefined');
    if (wwLib.wwPlugins) {
      debug.log('Available wwLib plugins:', Object.keys(wwLib.wwPlugins));

      // Check specific Supabase plugin
      if (wwLib.wwPlugins.supabase) {
        const plugin = wwLib.wwPlugins.supabase;
        debug.log('✅ wwLib.wwPlugins.supabase found!');
        debug.log('Plugin properties:', Object.keys(plugin));
        debug.log('Plugin has instance:', !!plugin.instance);
        debug.log('Plugin has callPostgresFunction:', typeof plugin.callPostgresFunction === 'function');
        debug.log('Plugin has settings:', !!plugin.settings);

        if (plugin.settings) {
          debug.log('Plugin publicData:', !!plugin.settings.publicData);
          debug.log('Plugin privateData:', !!plugin.settings.privateData);
          if (plugin.settings.publicData) {
            debug.log('Project URL configured:', !!plugin.settings.publicData.projectUrl);
            debug.log('API Key configured:', !!plugin.settings.publicData.apiKey);
          }
        }

        if (plugin.instance) {
          debug.log('Instance properties:', Object.keys(plugin.instance));
          debug.log('Instance has rpc method:', typeof plugin.instance.rpc === 'function');
        }
      } else {
        debug.log('❌ wwLib.wwPlugins.supabase not found');
      }
    }

    // Check store
    debug.log('wwLib.$store available:', typeof wwLib.$store !== 'undefined');
    if (wwLib.$store) {
      debug.log('Store state available:', !!wwLib.$store.state);
      if (wwLib.$store.state && wwLib.$store.state.data) {
        debug.log('Store data available:', !!wwLib.$store.state.data);
        if (wwLib.$store.state.data.plugins) {
          debug.log('Available plugins in store:', Object.keys(wwLib.$store.state.data.plugins));
        }
      }
    }

    // Check wwVariable
    debug.log('wwLib.wwVariable available:', typeof wwLib.wwVariable !== 'undefined');
    if (wwLib.wwVariable) {
      try {
        const supabaseVar = wwLib.wwVariable.getValue('supabase');
        debug.log('wwLib.wwVariable.getValue("supabase"):', !!supabaseVar);
      } catch (err) {
        debug.log('Error accessing wwVariable supabase:', err.message);
      }
    }
  }

  // Check window global
  debug.log('window.wwPlugins available:', typeof window !== 'undefined' && !!window.wwPlugins);
  if (typeof window !== 'undefined' && window.wwPlugins) {
    debug.log('Available window plugins:', Object.keys(window.wwPlugins));
  }

  debug.log('window.supabase available:', typeof window !== 'undefined' && !!window.supabase);
  ...
  try {
    ...
    if (typeof wwLib !== 'undefined' && wwLib.wwPlugins && wwLib.wwPlugins.supabase) {
      ...
      if (supabasePlugin.instance) {
        debug.log('✅ Found Supabase client via wwLib.wwPlugins.supabase.instance');
      ...
      if (supabasePlugin.callPostgresFunction) {
        debug.log('✅ Found Supabase plugin via wwLib.wwPlugins.supabase (using plugin methods)');
    ...
    if (typeof wwLib !== 'undefined' && wwLib.wwPlugins) {
      ...
      for (const path of possiblePaths) {
        if (wwLib.wwPlugins[path] && wwLib.wwPlugins[path].instance) {
          debug.log(`✅ Found Supabase via wwLib.wwPlugins.${path}.instance`);
      ...
      try {
        ...
        if (store.state && store.state.data && store.state.data.plugins) {
          ...
          if (supabasePlugin && supabasePlugin.instance) {
            debug.log('✅ Found Supabase via wwLib.$store.state.data.plugins');
            return supabasePlugin.instance;
          }
        }
      } catch (err) {
        debug.log('❌ Failed to access via wwLib.$store:', err.message);
      }
    }
      ...
      try {
        ...
        for (const varName of possibleVariables) {
          ...
          if (supabaseVar && (supabaseVar.instance || supabaseVar.rpc)) {
            debug.log(`✅ Found Supabase via wwLib.wwVariable.getValue('${varName}')`);
            return supabaseVar.instance || supabaseVar;
          }
        }
      } catch (err) {
        debug.log('❌ Failed to access via wwLib.wwVariable:', err.message);
      }
    }

    // Method 5: Global window fallbacks
    if (typeof window !== 'undefined' && window.wwPlugins && window.wwPlugins.supabase) {
      debug.log('✅ Found Supabase via window.wwPlugins.supabase');
      return window.wwPlugins.supabase.instance || window.wwPlugins.supabase;
    }

    if (typeof window !== 'undefined' && window.supabase) {
      debug.log('✅ Found Supabase via window.supabase');
      return window.supabase;
    }

    // All methods failed
    debug.warn('⚠️ WeWeb Supabase plugin not found in any location');
    debug.warn('Available wwLib properties:', typeof wwLib !== 'undefined' ? Object.keys(wwLib) : 'wwLib not available');
    debug.warn('Please ensure:');
    debug.warn('1. WeWeb Supabase plugin is installed and configured');
    debug.warn('2. Plugin has valid projectUrl and apiKey settings');
    debug.warn('3. Plugin has been initialized (check wwLib.wwPlugins.supabase.instance)');
    debug.warn('Component will work without Supabase but geographic features will be disabled');

    return null; // Return null instead of throwing error

  } catch (error) {
    debug.error('Error accessing Supabase client:', error);
    return null; // Return null instead of throwing error
  }
}
...
export async function testSupabaseConnection() {
  try {
    debug.group('🧪 Testing Supabase GIS Connection');

    const supabase = getSupabaseClient();
    debug.log('Supabase client obtained:', !!supabase);
    ...
    if (error) {
      debug.error('GIS schema access failed:', error);
      return { success: false, error: error.message };
    }

    debug.log('✅ GIS schema access successful');
    return { success: true, dataCount: data?.length || 0 };

  } catch (error) {
    debug.error('❌ Supabase GIS connection test failed:', error);
    return { success: false, error: error.message };
  } finally {
    debug.groupEnd();
  }
...
export const boundaryAPI = {
  ...
  async getCountriesInBounds(bounds, zoomLevel = 1) {
    try {
      ...
      if (typeof supabase.rpc === 'function' && bounds) {
        try {
          debug.log('🎯 Calling get_simplified_boundaries_in_bbox for countries with spatial filter');
          ...
          if (error) {
            debug.error('❌ Spatial RPC failed for countries:', error);
          } else if (data && data.length > 0) {
            debug.log('✅ Spatial RPC successful, returned', data.length, 'countries (simplified + filtered)');
            ...
            debug.log('⚠️ Spatial RPC returned no countries for current bounds');
            return [];
          }
        } catch (rpcError) {
          debug.error('❌ Spatial RPC call failed for countries:', rpcError);
        }
      }

      // Fallback to direct table query if RPC fails
      debug.log('🔄 Falling back to RPC function for gis.countries');
      ...
      if (error) {
        debug.error('❌ Error fetching countries via RPC:', error);

        // Try basic select as last resort
        debug.log('🔄 Trying basic select with raw geometry');
        ...
        if (rawError) {
          debug.error('❌ Error fetching countries from gis schema:', rawError);
          return [];
        }

        debug.warn('⚠️ No geometry data available - countries will not render');
        return [];
      }

      if (!data || data.length === 0) {
        debug.warn('⚠️ No countries data returned from gis schema');
        return [];
      }

      debug.log('✅ Direct table query successful, returned', data.length, 'countries');
      return data.map(country => ({
        id: country.id,
        name: country.name,
      ...
      debug.error('Boundary API error (getCountriesInBounds):', err);
      return [];
    }
  },
  ...
  async getStatesInBounds(bounds, zoomLevel = 6, countryFilter = null) {
    try {
      ...
      if (typeof supabase.rpc === 'function' && bounds) {
        try {
          debug.log('🎯 Calling get_simplified_boundaries_in_bbox for states with spatial filter');
          ...
          if (error) {
            debug.error('❌ Spatial RPC failed for states:', error);
          } else if (data && data.length > 0) {
            debug.log('✅ Spatial RPC successful, returned', data.length, 'states (simplified + filtered)');
            ...
            debug.log('⚠️ Spatial RPC returned no states for current bounds');
            return [];
          }
        } catch (rpcError) {
          debug.error('❌ Spatial RPC call failed:', rpcError);
        }
      }

      // Fallback to direct table query (without spatial filtering)
      debug.log('🔄 Falling back to RPC function for states');
      ...
      if (error) {
        debug.error('❌ Error fetching states via RPC:', error);
        ...
        if (countryFilter) {
          ...
          debug.log('🔍 Filtering states by country:', countryFilter);
        }

        const { data: rawData, error: rawError } = await query;

        if (rawError) {
          debug.error('❌ Error fetching states from gis schema:', rawError);
          return [];
        }

        debug.warn('⚠️ No geometry data available - states will not render');
        return [];
      }

      if (!data || data.length === 0) {
        debug.warn('⚠️ No states data returned from gis schema');
        return [];
      }

      debug.log('✅ GIS schema query successful, returned', data.length, 'states');
      return data.map(state => ({
        id: state.id,
        name: state.name,
      ...
      debug.error('Boundary API error (getStatesInBounds):', err);
      return [];
    }
  },
  ...
  toGeoJSON(boundaries) {
    debug.log('🔄 Converting boundaries to GeoJSON:', boundaries?.length || 0, 'boundaries');
    ...
    const geoJsonData = {
      ...
      features: boundaries.map(boundary => {
        try {
          ...
          debug.error('Error parsing geometry for boundary:', boundary.name, error);
          return null;
        }
      }).filter(f => f !== null)
    };

    debug.log('✅ GeoJSON created with', geoJsonData.features.length, 'valid features');
    return geoJsonData;
  }
};
